...
```
As we can see, it is rounding down by each year, beginning with January 1st and omitting the time. It also produces a large number of entries, so we will only be looking at the head and tail results of the following examples from this point on. 
